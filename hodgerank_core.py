...
def plot_graph(items, edges, weightedges, residuals,
    ...
    node_numbers = {node: i+1 for i, node in enumerate(items)}
    ...
    def plot_weighted_graph(pos):
        ...
        widths = [0.4 + 2.5 * (w / max_w) for w in edge_weights]
        alphas = [0.3 + 0.7 * (w / max_w) for w in edge_weights]
        ...
        for node, (x, y) in pos.items():
            ax.text(x + labels1[0], y + labels1[1], abbrev_map[node] if layout1 != "default" else str(node_numbers[node]), fontsize=8, ha="center")

        for (u, v), lw, a, w in zip(edges, widths, alphas, edge_weights):
            color = cmap(norm(w))
            arrow = FancyArrowPatch(pos[u], pos[v], connectionstyle=f"arc3,rad={random.choice([-0.25,0.25])}",
                                    arrowstyle="-", color=color, linewidth=lw, alpha=a)
            ax.add_patch(arrow)

        sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
        sm.set_array([])
        fig.colorbar(sm, ax=ax, fraction=0.046, pad=0.04, label="Comparisons")
        if layout1 == "default":
            legend_text = "\n".join([f"{node_numbers[n]}. {n}" for n in items])
            fig.text(1.05, 0.5, legend_text, va="center", ha="left", fontsize=9)

            ax.axhline(y=np.mean(freqs), color="black", linestyle="--")
            ax.axvline(x=np.mean(potentials), color="black", linestyle="--")
    ...
    def plot_residual_graph(pos):
        def draw_residual_arrow(ax, u, v, res, color, lw, alpha):
            ...
            arrow = FancyArrowPatch(start, end, connectionstyle=f"arc3,rad={random.choice([-0.25,0.25])}",
        ...
        widths = [0.25 + 3.0 * (abs(v) / max_w) for v in edgeresidual]
        alphas = [0.01 + 0.8 * (abs(v) / max_w) for v in edgeresidual]
