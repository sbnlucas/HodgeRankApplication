...
def export_to_csv(data:pd.DataFrame, name:str):
    ...
    data.round(4).to_csv(file)
    return file

def export_to_excel(data:pd.DataFrame, name:str):
    file = os.path.join(st.session_state.get("excel_folder"), name)
    data.round(4).to_excel(file)
    return file

def export_to_pdf(figure, name:str):
    pdf_file = os.path.join(st.session_state.get("pdf_folder"), name)
    figure.savefig(pdf_file, format="pdf", dpi=300, bbox_inches="tight")
    return pdf_file

def export_to_png(figure, name:str):
    png_file = os.path.join(st.session_state.get("png_folder"), name)
    figure.savefig(png_file, format="png", dpi=300, bbox_inches="tight")
    return png_file
...
def _make_export_dirs() -> dict:
    base = Path.home() / "HodgeRank_Exports"
    ...
    return {
        "folder": str(base),
        "pdf_folder": str(base / "PDF"),
        "csv_folder": str(base / "CSV"),
        "excel_folder": str(base / "Excel"),
        "png_folder": str(base / "PNG"),
    }
...
if "step" not in st.session_state:
    ...
    st.session_state.update(_make_export_dirs())
...
# =========================================================================================================================
    ...
    st.markdown("""
        ...
        This step computes the global ranking (potentials) of items based on the evaluations provided.
         ...
         Please note: computation time may vary depending on your machine's processing power and the size of the dataset.
    """)
...
if step == 5:
    ...
    with col1: 
        ...
        if left.button("Export the Ranking Table to .csv"): 
            path = export_to_csv(ranking_table, "RankingTable.csv")
            st.success(f"Sucessfully! Exported to {path}")
        if right.button("Export the Ranking Table to excel (.xlsx)"):
            path = export_to_excel(ranking_table, "RankingTable.xlsx")
            st.success(f"Sucessfully! Exported to {path}")
    ...
    with left: 
        ...
        with s1:
            if st.session_state["fig_pie"] is not None:
                ...
                if exp1.button("Export figure to PDF"):
                    path = export_to_pdf(fig, "Decomposition.pdf")
                    st.success(f"Sucessfully! Exported to {path}")
                if exp2.button("Export figure to PNG"):
                    path = export_to_png(fig, "Decomposition.png")
                    st.success(f"Sucessfully! Exported to {path}")
...
if step == 7:
    ...
    with col1:
        ...
        with s1:
            ...
            if exp1.button("Export figure to PDF"):
                path = export_to_pdf(fig, "ScatterPlot.pdf")
                st.success(f"Sucessfully! Exported to {path}")
            if exp2.button("Export figure to PNG"):
                path = export_to_png(fig, "ScatterPlot.png")
                st.success(f"Sucessfully! Exported to {path}")
        with s2:
...
if step == 8:
    ...
    with col1:
        ...
        with s1:
            ...
            if exp1.button("Export graphs to PDF"):

                name = "WeightedGraph_" + type_plot[st.session_state["type_plotw"]]
                name2 = "ResidualGraph_" + type_plot[st.session_state["type_plotr"]] + st.session_state["Highlight"]

                path = export_to_pdf(fig1, name +".pdf")
                export_to_pdf(fig2, name2 + ".pdf")

                st.success(f"Sucessfully! Exported to {path}")
            
            if exp2.button("Export graphs to PNG"):
                
                name = "WeightedGraph_" + type_plot[st.session_state["type_plotw"]]
                name2 = "ResidualGraph_" + type_plot[st.session_state["type_plotr"]] + st.session_state["Highlight"]

                path = export_to_png(fig1, name + ".png")
                export_to_png(fig2, name2 + ".png")
...
if step == 9:
    ...
    with right:
        ...
        with st.expander("⚙️ Customize matrices plots", expanded=False):
            st.markdown("""You can choose which part of the matrix to display in the heatmap.  
        ...
        with s1:
            ...
            exp1, exp2 = st.columns([0.5,1])
            if exp1.button("Export matrices to PDF"):
                path = export_to_pdf(fig1, "FlowMatrixHeatMap.pdf")
                export_to_pdf(fig2, "PotentialMatrixHeatMap.pdf")
                export_to_pdf(fig3, "ResidualsMatrixHeatMap.pdf")

                st.success(f"Sucessfully! Exported to {path}")
            if exp2.button("Export matrices to PNG"):
                path = export_to_png(fig1, "FlowMatrixHeatMap.png")
                export_to_png(fig2, "GradientMatrixHeatMap.png")
                export_to_png(fig3, "ResidualsMatrixHeatMap.png")
...
if step == 10:
    ...
    if "results" in st.session_state:
        ...
        *The global ranking assigns a **potential score** to each item.  
        ...
        st.table(bottom3[["Rank","Potential Score", "Evaluation Frequency (%)"]])

        unstable = ranking_table[
            (ranking_table["Potential Score"] > ranking_table["Potential Score"].median()) &
            (ranking_table["Evaluation Frequency (%)"] < 10)]
        if not unstable.empty:
            st.warning("""
            ⚠️ Some items appear as **unstable**.  
            This does not mean their ranking is incorrect, but that they have high potential  
        ...
        *These are the pairwise comparisons where the observed outcomes  
        ...
        if left1.button("Export the full table to .csv"): 
            path = export_to_csv(edge_df, "InconsistentEdges.csv")
            st.success(f"Sucessfully! Exported to {path}")
        if right1.button("Export the full table to excel (.xlsx)"):
            path = export_to_excel(edge_df, "InconsistentEdges.xlsx")
            st.success(f"Sucessfully! Exported to {path}")
        ...
        **Interpretation:**  
        ...
        if not results["curlvalues"].empty:
            ...
            if left2.button("Export table to .csv"): 
                path = export_to_csv(tri_df, "InconsistentTriangles.csv")
                st.success(f"Sucessfully! Exported to {path}")
            if right2.button("Export table to excel (.xlsx)"):
                path = export_to_excel(tri_df, "InconsistentTriangles.xlsx")
                st.success(f"Sucessfully! Exported to {path}")
        else:
        ...
        # ==========================================================
        st.subheader("4. Automatic Interpretation")
        st.markdown("""
